...
def panel_success(value: tuple, operation: str) -> Panel:
    ...
    match operation:
        case "addition":
            panel = Panel(addition_success,
                          title="[title_success]INFO", style="success")
        case "deletion":
            panel = Panel(deletion_success,
                          title="[title_success]INFO", style="success")
...
def panel_not_found_data(data, case: str) -> Panel:
    ...
    match case:
        case "nama":
            panel = Panel(Padding(Text(f"Mahasiswa dengan nama [{data}] tidak ditemukan!", justify="center", style="text_warning"), pad=(
                1, 0, 1, 0)), title="[title_warning]INFO", style="warning")
        case "nim":
            panel = Panel(Padding(Text(f"Mahasiswa dengan NIM [{data}] tidak ditemukan!", justify="center", style="text_warning"), pad=(
                1, 0, 1, 0)), title="[title_warning]INFO", style="warning")
...
def search_data(data, linked_list: SLinkedList, based_on: str):
    ...
    result_table.add_column(
        "[text_title]No.", style="text_default", justify="center")
    result_table.add_column(
        "[text_title]NIM", style="text_default", min_width=20)
    result_table.add_column(
        "[text_title]Nama", style="text_default", min_width=20)
            ...
            for data_mhs in linked_list.traverse():
                if data_mhs[0] == data:
                    result_table.add_row(
                        f"{count}", f"{data_mhs[1]}", f"{data_mhs[0]}")
                    console.print(result_table, justify="center")
                    return
                count += 1

            console.print(Padding(panel_not_found_data(
                data, case="nama"), pad=(1, 0, 0, 0)))
...
def main():
    ...
    panel_menu = Panel(
        menu_str, title="[text_title]Menu Program", title_align="left", style="default")
    panel_description = Panel(
        program4.description, title="[text_title]Deskripsi Program", title_align="left", style="default")
    panel_searching_options = Panel(
        searching_options_str, title="[text_title]Opsi Pencarian Data", title_align="left", style="default")
    panel_deletion_options = Panel(
        deletion_options_str, title="[text_title]Opsi Penghapusan Data", title_align="left", style="default")
    ...
    while True:
        ...
        opt = IntPrompt.ask("\n[bold]Pilih Menu", choices=[
                            str(i) for i in menu.keys()])
        ...
        match opt:
            case 1:  # case 1: Tambah data
                console.clear()
                console.rule(
                    "[text_title]Prompt Penambahan Data", style="default")

                while True:
                    nama = Prompt.ask("[bold]\nMasukkan Nama Mahasiswa")
                    if not nama.replace(" ", "").isalpha():
                        console.print(
                            "[prompt.invalid]Nama hanya boleh berisi karakter alphabet!")
                        continue
                    break
                ...
                while True:
                    ...
                    if not nim.isdecimal():
                        console.print(
                            "[prompt.invalid]NIM hanya boleh berisi angka!")
                        continue
                    ...
                    for data in linked_list.traverse():
                        if data[1] == nim:
                            console.print(
                                "[prompt.invalid]NIM yang dimasukkan sudah digunakan")
                            available = False
                            break
                ...
                console.print(Padding(panel_success(
                    (nama, nim), operation="addition"), pad=(1, 0, 0, 0)))

                getpass.getpass("\nKlik 'Enter' untuk melanjutkan")

            case 2:  # case 2: Tampilkan data
                if linked_list.empty():
                    console.print(Padding(panel_empty_data(
                        operation="display_data"), pad=(1, 0, 0, 0)))
                else:
                    console.clear()
                    console.rule(
                        "[text_title]Tabel Data Mahasiswa", style="default")
                    console.print(Padding(table_data(linked_list),
                                  pad=(1, 0, 0, 0)), justify="center")
            ...
            case 3:  # case 3: Cari data
                ...
                console.print(
                    Padding(panel_searching_options, pad=(1, 0, 0, 0)))

                opt = IntPrompt.ask("\n[bold]Pilih opsi pencarian data", choices=[
                                    str(i) for i in searching_options.keys()])
            ...
            case 4:
                if linked_list.empty():
                    console.print(Padding(panel_empty_data(
                        operation="deletion"), pad=(1, 0, 0, 0)))
                    getpass.getpass("\nKlik 'Enter' untuk melanjutkan")
                    continue
                ...
                console.print(
                    Padding(panel_deletion_options, pad=(1, 0, 0, 0)))

                opt = IntPrompt.ask("\n[bold]Pilih opsi Penghapusan data", choices=[
                                    str(i) for i in deletion_options.keys()])
...
title = "[text_title]Program 4: Implementasi Linked List"
