    ...
    match operation:
        case "addition":
            panel = Panel(Text(f"\nData [{value}] telah dimasukkan ke antrian!\n", justify="center",
                          style="text_success"), title="[title_success]INFO", style="success")
        case "deletion":
...
def not_found_data_panel(data) -> Panel:
    ...
    panel = Panel(Padding(Text(f"Data [{data}] tidak ada di dalam antrian!", justify="center", style="text_warning"), pad=(
...
def table_data(data: Array, opt: str, searching_data=None) -> Table:
    ...
    match opt:
        ...
        case "full_data_ascending":
            ...
            data_ascending = sorted(
                sorted(data._array), key=lambda s: s.lower())
            for i in range(data.size()):
                table.add_row(f"{i+1}", data_ascending[i])
        case "full_data_descending":
            table.title = "[text_title]Data pada Antrian (Descending)"
            data_descending = sorted(
                sorted(data._array, reverse=True), key=lambda s: s.lower(), reverse=True)
            for i in range(data.size()):
                table.add_row(f"{i+1}", data_descending[i])
        case "searching_data":
            table.title = "[text_title]Hapus Data"
            table.add_row(
                f"{data._array.index(searching_data)+1}", searching_data)
        case "top_data":
...
def main():
    ...
    panel_delete_opt = Panel(
        delete_opt_str, title="[text_title]Opsi Hapus Data", title_align="left", style="default")
    panel_searching_opt = Panel(
        display_opt_str, title="[text_title]Opsi Tampilan Data", title_align="left", style="default")
    ...
    while True:
        ...
        console.print(
            "[prompt.invalid]Harap masukkan bilangan bulat positif (lebih besar dari 0)!")
        ...
        match opt:
            case 1:
                while True:
                    ...
                    console.print(
                        "[prompt.invalid]Input data tidak boleh kosong!")
            ...
            case 2:
                ...
                opt = IntPrompt.ask("\n[bold]Pilih opsi untuk menghapus data", choices=[
                                    str(i) for i in delete_opt.keys()])

                match opt:
                    case 1:
                        console.print(table_data(
                            array, opt="top_data"), justify="center")
            ...
            case 3:
                ...
                opt = IntPrompt.ask("\n[bold]Pilih opsi untuk menampilkan data", choices=[
                                    str(i) for i in display_opt.keys()])

                match opt:
                    case 1:
                        console.print(table_data(
                            array, opt="full_data_ascending"), justify="center")
                    case 2:
                        console.print(table_data(
                            array, opt="full_data_descending"), justify="center")
                    case 3:
                        console.print(table_data(
                            array, opt="full_data_normal"), justify="center")
...
# untuk di tampilkan di list menu
name = "Implementasi Antrian dengan Array (2)"
